...
def add_lot_blood(
    date: Date, liters: float, bank_name: str, donor: int
) -> Optional[int]:
    ...
    with Session(engine) as session:
        ...
        query = (
            session.query(Donor).add_column(Bank.id).filter(Donor.id == donor).first()
        )
    ...
    with Session(engine) as session:
        try:
            lot_blood = LotBlood(
                date=date, liters=liters, bank_id=bank_id, donor_id=donor_id
            )
